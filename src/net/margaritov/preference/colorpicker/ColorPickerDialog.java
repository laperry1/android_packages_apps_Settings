...
public class ColorPickerDialog extends Dialog implements
    ...
    public ColorPickerDialog(Context context, int theme, int initialColor,
            int androidColor, int SamadhiColor) {
        super(context, theme);
        mInitialColor = initialColor;
        mAndroidColor = androidColor;
        mSamadhiColor = samadhiColor;
        if (mAndroidColor != 0x00000000 && mSamadhiColor != 0x00000000) {
    ...
    public boolean onMenuItemClick(MenuItem item) {
        if (item.getItemId() == R.id.palette_samadhi) {
            mPalette = PALETTE_SAMADHI;
            mColorTransitionAnimator.start();
            return true;
            ...
            return true;
        } else if (item.getItemId() == R.id.reset_samadhi) {
            mColorPicker.setColor(mSamadhiColor, true);
            return true;
        }
        return false;
    }
